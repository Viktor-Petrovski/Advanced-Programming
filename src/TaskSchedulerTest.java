...
public class TaskSchedulerTest {
    ...
    static class TaskRunner<T extends Task>{
        public void run(TaskScheduler<T> scheduler, T[] tasks) {
            List<T> order = scheduler.schedule(List.of(tasks));
            order.forEach(System.out::println);
        }
    }

    interface TaskScheduler<T extends Task> {
        List<T> schedule(List<T> tasks);
    }
    ...
    static class PriorityTask implements Task {
    ...
    static class TimedTask implements Task {
    ...
    static class Schedulers {
        ...
        static TaskScheduler<Task> getOrdered() {
            return l -> l.stream().sorted(Comparator.comparing(Task::getOrder))
                    .collect(Collectors.toCollection(ArrayList::new));
        }

        static TaskScheduler<Task> getFiltered(int order) {
            return l -> l.stream().filter(t -> t.getOrder() <= order)
                    .collect(Collectors.toCollection(ArrayList::new));
        }
    }
}
