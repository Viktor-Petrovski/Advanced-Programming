...
public class BankTester {
    ...
    static class Account {
        ...
        private static final Set<Long> idSet;
        ...
        public Account(String name, double balance) {
            ...
            while (true) {
                long candidateId = RAND.nextLong();
                if (!idSet.contains(candidateId)) {
                    id = candidateId;
                    idSet.add(id);
                    break;
                }
    ...
    static class Bank {
        ...
        private Account getAccount(long id) {
            return Arrays.stream(accounts).filter(a -> a.getId() == id).findAny().orElse(null);
        }

        /// врши проверка дали корисникот ги има потребните средства на сметка и дали и двете сметки на кои се однесува трансакцијата
        /// се нависитина во банката и ако и двата услови се исполнето ја извршува трансакцијата и враќа true, во спротивно враќа false
        boolean makeTransaction(Transaction t) {
            ...
            if (from.getBalance() < provision + amount)
                return false;

            from.setBalance(from.getBalance() - (amount + provision));
        ...
        public String toString() {
            ...
            Arrays.stream(accounts).forEach(sb::append);
