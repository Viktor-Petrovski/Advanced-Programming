...
class HighExpertiseEvaluator implements DoctorEvaluator {
    ...
    public boolean evaluate(Doctor doctor) {
        return doctor.getLevel() >= 7 && doctor.getPatients() > 20;
    }
}
...
public class HospitalDemo {
    public static void main(String[] args) {
        ...
        Supplier<Doctor> docReader = () -> {
            String[] s = sc.nextLine().split(" ");
            return new Doctor(Integer.parseInt(s[0]), s[1], Integer.parseInt(s[2]), Integer.parseInt(s[3]));
        };
        ...
        Doctor[] passing = er.evaluate(new HighExpertiseEvaluator());
        ...
        Predicate<Doctor> isNotChief = d -> d.getLevel() < 10;
        Consumer<Doctor> lvlUp = d -> d.setLevel(d.getLevel() + 1);
        er.conditionalMutate(isNotChief, lvlUp);
        er.forEach(System.out::println);
        ...
        Predicate<Doctor> cond = d -> isNotChief.test(d) && d.getPatients() >= 30;
        er.conditionalMutate(cond, lvlUp);

        //TODO: Map doctors to labels in the format: Name: name, Level: level
        System.out.println("\n=== Map doctors to labels ===");
        Function<Doctor, String> mapper = d -> "Name: " + d.getName() + ", Level: " + d.getLevel();
        String [] res = er.mapToLabels(mapper);
